...
/* nimble-rs: vendored copy of esp-nimble 039d2d62
 * porting/nimble/src/os_mempool.c with 64-bit-pointer correctness fixes (the
 * original truncates pointers through uint32_t, crashing os_mempool_clear /
 * os_memblock_from on 64-bit hosts; the upstream Makefile forces -m32 for the
 * same reason). Compiled *instead of* the original (see gen/builder.rs); the
 * paired os/os_mempool.h in glue widens mp_membuf_addr to uintptr_t.
 * Keep in sync on submodule bumps. */
