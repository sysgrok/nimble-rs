...
/* nimble-rs: vendored copy of esp-nimble's os/os_mempool.h with a single
 * 64-bit-correctness fix (see mp_membuf_addr below). Keep in sync on submodule
 * bumps. */
...
struct os_mempool {
    ...
    /* nimble-rs: widened from uint32_t - storing a pointer in a u32 truncates
     * on 64-bit hosts, crashing os_mempool_clear() (which rebuilds the free
     * list from this field) at deinit time. This header is otherwise a
     * verbatim copy of esp-nimble 039d2d62 porting/nimble/include/os/os_mempool.h,
     * shadowing it via the glue include directory. */
