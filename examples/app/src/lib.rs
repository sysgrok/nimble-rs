//! The portable part of the examples: a GATT server with a write
//! characteristic and an indicate characteristic pushing a counter to
//! subscribers once a second - the same scenario for every target; only the
//! controller (and optionally a [`Parker`]) differ.
...
use nimble_rs::{gatt_services, Ble, BleError, BleUuid, ConnHandle, Controller, HostEvent, Parker};

// Logging front-end: `log` and/or `defmt`, whichever the target example
// enables; nothing otherwise.
macro_rules! info {
    ($s:literal $(, $arg:expr)* $(,)?) => {{
        #[cfg(feature = "log")]
        ::log::info!($s $(, $arg)*);
    ...
    match event {
        ...
        GattsEvent::Write {
            ...
            match data.read(&mut buf) {
                ...
                Err(e) => warn!("recv read failed (error {})", e.code()),
            }
        }
...
pub async fn gatt_server<C: Controller>(controller: C, parker: Option<&dyn Parker>) -> ! {
        ...
        loop {
            ...
            if needs_adv_swap(false) {
                match start_advertising(&ble) {
                    Ok(()) => info!("advertising"),
                    Err(e) => warn!("failed to start advertising (error {})", e.code()),
                }
            }
            ...
            for conn in subs {
                if let Err(e) = ble.indicate(conn, ind_handle, &counter.to_le_bytes()) {
                    warn!("indicate failed (error {})", e.code());
                }
            }
        }
    })
